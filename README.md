...
- **GPU-driven forward rendering with VK_EXT_device_generated_commands (GD6)** — `GpuCullPass` compute shader frustum-culls instances each frame; outputs `compactInstanceList[]` + `indirectDrawBuf{visibleCount,1,1}` + `dgcBuf` (kMaxInstances × `{1,1,1}`); `vkCmdExecuteGeneratedCommandsEXT` dispatches N per-instance sequences; task shader uses `compactInstanceList[gid.x + SV_DrawIndex]` — single entry point for both DGC and GD3 fallback — CPU records only, no readback
