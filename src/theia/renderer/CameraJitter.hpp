...
inline float radicalInverse(std::uint32_t index, std::uint32_t base) noexcept {
...
inline sm::float2 halton23(std::uint32_t sampleIndex) noexcept {
    const std::uint32_t seqIndex = sampleIndex + 1U; // avoid (0,0)
    return sm::float2{
...
inline sm::float2 cameraJitterPixels(std::uint32_t sampleIndex) noexcept {
    return halton23(sampleIndex) - sm::float2{0.5f, 0.5f};
}

/// Convert pixel-footprint jitter to NDC offsets.
inline sm::float2 cameraJitterNdc(std::uint32_t sampleIndex, std::uint32_t width, std::uint32_t height) noexcept {
