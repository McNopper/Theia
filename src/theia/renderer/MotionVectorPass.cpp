...
bool MotionVectorPass::createPipeline(const char* spvName) noexcept {
    ...
    m_ctx->setDebugName(
        VK_OBJECT_TYPE_PIPELINE, reinterpret_cast<uint64_t>(m_pipeline), "theia.motionVectors.pipeline");
    return true;
}
...
void MotionVectorPass::record(VkCommandBuffer cmd, const FrameParams& params) noexcept {
    ...
    const VkImageMemoryBarrier2 preMvBarrier{
        ...
        .srcStageMask = m_firstUse ? VK_PIPELINE_STAGE_2_NONE : VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
